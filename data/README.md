...
- Kakao Developers를 이용해 문자열 주소 데이터를 위도(Latitude) 및 경도(Longitude) 좌표계로 변환하여 공간 데이터화 완료
- data4의 경우 12,383 데이터 중, 좌표가 정상적으로 생성된 데이터는 12,284개. 99개 데이터는 주소 불분명 등의 이유로 좌표 정보가 포함되지 않았으나, 나머지 정보는 모두 포함되어 있음
...
- 웹 페이지 내 불필요한 노이즈(메뉴바, 푸터, 숨김 태그 등)를 제거하고 핵심 본문(Content)만 추출

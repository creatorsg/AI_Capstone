"""OpenAI API 기반 AI 응답 생성 (인텐트별 RAG 프롬프트 + multi-turn 지원)
...
파이프라인:
  ...
  [4] 프롬프트 포매팅 → OpenAI API 호출 (system + history + user)
...
def get_ai_response_openai(
    ...
    # ── OpenAI messages 배열: system + history(최근 N턴) + user ─────────────
